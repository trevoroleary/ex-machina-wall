...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            starting_address = randint(0, 70)
            packet = LEDPacket(starting_address, 100, [(rand_int(100), rand_int(100), rand_int(100)) for i in range(10)])
            # packet = LEDPacket(i*10, 100, [color for i in range(10)])
            payload = packet.get_payload()
            send_packet(payload)
            # time.sleep(0.1)
            time.sleep(abs(normalvariate(0, 0.1)))
